...
$client = new Vbb_Client( 'pedelecs.co.uk/forum',
	...
	$forum_feed = $client->call( 'forumdisplay', array( 'forumid' => 2) );
	...
	foreach($display as $post){	
		...
		$excerpt = substr($post->preview, 0, 100).'...';
		
		echo '<h5 style="margin-bottom: 6px;"><a href="http://www.pedelecs.co.uk/forum/showthread.php?t='.$post->threadid.'">'.$post->threadtitle.'</a></h5>';
		echo '<p style="margin: 0 0 6px 0;">'.$excerpt.'</p>';
		//echo '<p style="font-size: 12px;">'.$post->replycount.' comments. Last commented '.date("G:i d M, Y",$post->lastposttime).'</p>';
